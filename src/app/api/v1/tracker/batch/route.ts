...
import { trackBatchEvents } from '@/lib/services/tracking';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Track all events
    const result = await trackBatchEvents(auth.tenant_id, parseResult.data.events);

    return NextResponse.json(
      {
        success: true,
        events_processed: result.event_ids.length,
